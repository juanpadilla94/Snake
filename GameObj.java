...
public class GameObj {
	...
	public void resetPosition(){
		pos_x = (int) Math.floor(Math.random()*max_x);
		pos_y = (int) Math.floor(Math.random()*max_y);
	}
   ...
   if (theta <= diagTheta ) {
	...
	public boolean checkHitWall() {
		return (pos_x + v_x + 4 < 0 || pos_x + v_x - 4 > max_x || 
				pos_y + v_y + 4 < 0 || pos_y + v_y - 4 > max_y);
	}
	
	public boolean checkHitTrail() {
		for(Point p : trail) {
			if((p.getX() != 10 || p.getY() != 10) && (p.getX() == pos_x 
					&& p.getY() == pos_y)) {
				return true;
			}
		}
